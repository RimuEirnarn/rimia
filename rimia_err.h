...
#define throw(stuff) do { \
    stuff._raise(&stuff, __LINE__, __FILE__); \
} while (0)
// Throw error, now with dynamic reasoning.
#define throw2(stuff, reason) do {\
    stuff._raise2(&stuff, reason, __LINE__, __FILE__);\
} while (0)
// Throw error, pointer only?
#define throw_(stuff) do { \
    stuff->_raise(stuff, __LINE__, __FILE__); \
} while (0)
// Throw error, pointer only, with dynamic reasoning
#define throw2_(stuff, reason) do { \
    stuff->_raise2(stuff, __LINE__, __FILE__); \
} while (0)
// Throw error at default but with configurable reason.
#define raise(reason) do { \
    Exception exc = Exception_init(""); \
    throw2(exc, reason); \
} while (0)
// Throw an error with a reason, of a name
#define raise_from(name, reason) do { \
    Exception exc = Exception_init("");\
    exc.rename(&exc, name);\
    throw2(exc, reason); \
} while (0)
